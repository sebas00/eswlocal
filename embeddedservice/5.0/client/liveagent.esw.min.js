...
1;this.addCurrentPage();this.serialize()};n.prototype.serialize=function(){var a=JSON.stringify({visitCount:this.visitCount,originalReferrer:this.originalReferrer,pages:this.pages});embedded_svc?embedded_svc.setSessionData("LA_VISITOR_INFO",a):window.sessionStorage.setItem("LA_VISITOR_INFO",a)};n.prototype.getInfo=function(){return{visitCount:this.visitCount,originalReferrer:this.originalReferrer,pages:this.pages}};n.prototype.addCurrentPage=function(){var a=window.location.href;this.pages.forEach(function(c,
...
b.settings.entryFeature&&(b.settings.agentAvailableOnButtonClick=d,this.updateButton(d),b.inviteAPI&&b.inviteAPI.inviteButton.getTracker()&&b.inviteAPI.inviteButton.setOnlineState(d))}.bind(this))};g.prototype.handleSettings=function(a){var c;a.buttons.forEach(function(d){d.id===b.settings.buttonId&&(c=d.isAvailable,"Invite"===d.type&&(b.liveAgentAPI.inviteButton=d))});b.log("Agent Available: "+JSON.stringify(c));"LiveAgent"===b.settings.entryFeature&&(b.settings.agentAvailableOnButtonClick=c,this.updateButton(c));
embedded_svc.utils.fireEvent("onSettingsCallCompleted",void 0,{isAgentAvailable:c})};g.prototype.handleSwitchServer=function(a){a&&a.newUrl&&"string"===typeof a.newUrl?(b.log("Detected new Live Agent URL: "+a.newUrl+". Sending a request to new endpoint."),b.settings.baseLiveAgentURL=a.newUrl,this.connection.onSuccess(1),window.setTimeout(function(){this.getButtonSettings()}.bind(this),1)):b.error("Expected switch server response to return a redirect URL, instead received: "+(a&&a.newUrl?a.newUrl:
a)+".")};g.prototype.disconnect=function(){b.log("Disconnecting from Live Agent");this.connection.running=!1};g.prototype.isButtonId=function(a){return"573"===b.getKeyPrefix(a)};g.prototype.isDeploymentId=function(a){return"572"===b.getKeyPrefix(a)};g.prototype.sendFileInfo=function(a){var c=document.createEvent("CustomEvent");c.initCustomEvent("sendFileInfo",!0,!1,{name:a.name,size:a.size});window.dispatchEvent(c)};g.prototype.sendCustomEvent=function(a,c){var d={};d.type=a;d.data=c;b.postMessage("chasitor.sendCustomEvent",
d)};g.prototype.getCustomEvents=function(a){b.addMessageHandler("liveagent.getCustomEventsResult",a);b.postMessage("chasitor.getCustomEvents")};g.prototype.addCustomEventListener=function(a,c){b.addMessageHandler("liveagent.customEventReceived",c);b.postMessage("chasitor.addCustomEventListener",a)};g.prototype.getSessionId=function(){return this.sid};g.prototype.updateButton=function(a){b.settings.isOfflineSupportEnabled&&!a?(b.isButtonDisabled=!1,b.setHelpButtonText(b.settings.offlineSupportMinimizedText)):
b.isButtonDisabled=!a};g.prototype.handleInvitationsResource=function(){embedded_svc.config.invitation.htmlMarkup&&(b.liveAgentAPI.hasInvitationsLoaded=!0,embedded_svc.utils.fireEvent("onInvitationResourceLoaded"))};g.prototype.appendInvitationsComponent=function(){var a=document.createElement("div");a.id=embedded_svc.liveAgentAPI.INVITATIONS_CONTAINER_ID;a.innerHTML=embedded_svc.config.invitation.htmlMarkup;if(embedded_svc.config.invitation.styles){var c=document.createElement("style");c.type="text/css";
c.innerHTML=embedded_svc.config.invitation.styles;a.appendChild(c)}embedded_svc.settings.targetElement.appendChild(a);this.isInvitationsRendered=!0;embedded_svc.config.invitation.invitationAPIs&&(a=document.createElement("script"),a.id=embedded_svc.liveAgentAPI.INVITE_API_ID,a.type="text/javascript",a.innerHTML=embedded_svc.config.invitation.invitationAPIs,embedded_svc.settings.targetElement.appendChild(a));t(b.liveAgentAPI.inviteButton)};g.prototype.processInvitations=function(a){var c=embedded_svc.config.embeddedServiceConfig?
embedded_svc.utils.getSiteEndpointUrl(embedded_svc.config.embeddedServiceConfig):null;c?(embedded_svc.config.invitation={},embedded_svc.utils.loadScriptFromUrl(embedded_svc.utils.generateResourceUrl(c,a.resource),embedded_svc.liveAgentAPI.handleInvitationsResource,function(){embedded_svc.utils.error("[Invitations] Something went wrong while loading invitations static resource.")}.bind(embedded_svc),embedded_svc.liveAgentAPI.INVITE_RESOURCE_ID)):embedded_svc.utils.warning("Static resource cannot be loaded because no site endpoint exists for the embedded service deployment.")};
g.prototype.hasInviteButton=function(){return 0<Object.keys(b.liveAgentAPI.inviteButton).length&&b.liveAgentAPI.inviteButton.constructor===Object};g.prototype.startChat=function(a){if("function"!==typeof Promise)b.loadScriptFromDirectory("common","promisepolyfill",function(){return b.liveAgentAPI.startChat(a)},!0);else return(new Promise(function(c,d){var e;try{(e=document.querySelector(".embeddedServiceSidebar"))?e.classList.contains("sidebarMinimized")?e.dispatchEvent(new CustomEvent("embeddedservicemaximizechat",
{detail:{resolve:c,reject:d}})):e.classList.contains("sidebarMaximized")&&c("Embedded Service component is already maximized."):b.bootstrapEmbeddedService({chatAPISettings:a}).then(function(){b.liveAgentAPI.startChat.bind(b,a);c("Embedded Service application and component bootstrapped")}).catch(function(f){d(f)})}catch(f){d(f)}})).then(function(c){b.log("[Start Chat API] "+c);if(b.util.isChannelMenuContext()||b.menu)b.menu.hideButtonAndMenu(),b.menu.hideTopContainer();return(new Promise(function(d,
e){try{var f=document.querySelector(".embeddedServiceLiveAgentSidebarFeature");var m=document.querySelector(".embeddedServiceSidebarState");f?m.classList.contains("embeddedServiceLiveAgentStateWaiting")||m.classList.contains("embeddedServiceLiveAgentStateChat")?d("Restricted from starting chat in current state. Chat request will not fire."):(f.dispatchEvent(new CustomEvent("embeddedservicestartchat",{detail:{data:a?embedded_svc.validateStartChatAttributes(a):{},resolve:d,reject:e}})),b.addMessageHandler("liveagent.initialized",
function(){d("Successfully requested to start chat session.")})):!f&&document.getElementsByClassName("embeddedServiceSidebarDialogState")?d("Restricted from starting a chat in current state. Chat request will not fire."):d("Embedded Service sidebar feature not rendered. Start Chat was not called.")}catch(h){e(h)}})).then(function(d){b.log("[Start Chat API] "+d)}).catch(function(d){b.error("[Start Chat API] Error starting chat: "+d)})}).catch(function(c){b.error("[Start Chat API] Error bootstrapping/maximizing chat: "+
c)})};g.prototype.endChat=function(){if("function"!==typeof Promise)b.loadScriptFromDirectory("common","promisepolyfill",function(){return this.endChat()}.bind(this),!0);else return(new Promise(function(a,c){var d=["embeddedServiceLiveAgentStateWaiting","embeddedServiceLiveAgentStateChat","embeddedServiceSidebarDialogState"],e;try{(e=document.querySelector(".embeddedServiceSidebarState"))?d.some(function(f){return e.classList.contains(f)})?(e.dispatchEvent(new CustomEvent("embeddedserviceendchat",
{detail:{resolve:a,reject:c}})),b.addMessageHandler("session.deletedSessionData",function(f){-1!==f.indexOf("CHASITOR_SERIALIZED_KEY")&&-1!==f.indexOf("MASTER_DEPLOYMENT_ID")&&a("Successfully deleted session keys after chat ended.")})):(b.addMessageHandler("session.deletedSessionData",function(f){-1!==f.indexOf("ESW_IS_MINIMIZED")&&a("Successfully deleted session keys after client closed.")}),b.deleteSessionData(["ESW_IS_MINIMIZED"])):a("Embedded Service application state is not rendered on this page.")}catch(f){c(f)}})).then(function(a){b.log("[End Chat API] "+
